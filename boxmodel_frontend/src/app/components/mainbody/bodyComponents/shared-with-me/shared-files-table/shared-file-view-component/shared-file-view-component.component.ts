...
export class SharedFileViewComponentComponent implements OnInit {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: any,private sanitizer: DomSanitizer,private http:HttpClient,private dialogRef: MatDialogRef<SharedFileViewComponentComponent>,private toast: NgToastService) { }
  ...
  updateFile(){
    ...
    this.by=this.userToken.username
    this.on=this.fileUpdatingDate
    this.http.patch(`${this.NodeBackend}/${this.data.id}`,{file:uf,by:this.userToken.username,on:this.fileUpdatingDate})
    .subscribe(res=>{
     console.log(res)
     this.toast.success({detail:"SUCCESS",summary:'File updated Successully',duration:5000});
