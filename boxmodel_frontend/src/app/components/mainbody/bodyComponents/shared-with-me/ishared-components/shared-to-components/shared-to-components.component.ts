...
export class SharedToComponentsComponent implements OnInit {
  ...
  toggleChangesForUpdate(event:any,userName:any){
    ...
    this.http.patch(`${this.NodeBackend}/updateWriteAccess`,{st:userName,fid:this.data.fileId,nuv:event.checked})
    .subscribe(res=>{
     console.log(res)
     this.toast.success({detail:"SUCCESS",summary:`${userName}'s Write Access set to ${event.checked} `,duration:5000});
