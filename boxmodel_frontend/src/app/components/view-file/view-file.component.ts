...
export class ViewFileComponent implements OnInit {
  ...
  fileDetails?:File
  constructor(private router:Router,private route:ActivatedRoute,private fileService:FileService,private http:HttpClient,private sanitizer: DomSanitizer,public dialog: MatDialog) { }
    ...
    .subscribe((file:File)=>{
      ...
      var reader=new FileReader()
          ...
          reader.onload=(_event)=>{
            localStorage.setItem("BinaryFile",JSON.stringify(reader.result))
            this.binaryFile=this.sanitizer.bypassSecurityTrustResourceUrl(String(reader.result))
            console.log(this.binaryFile.changingThisBreaksApplicationSecurity.split(",")[1])

      
            let decodedString = atob(this.binaryFile.changingThisBreaksApplicationSecurity.split(",")[1]); 
  ...
  onUpdate(fileId:string|null){
    ...
    const url=`${this.SpringBackend}/${this.userToken.username}/update/id/${fileId}`
    this.http
